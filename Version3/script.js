...
function animateTriangles(fromStar, toStar, count, path) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        spaceship.scale.set(.5, .5, .5); // Adjust scale as needed
        scene.add(spaceship);
        ...
        movingTriangles.push({
            ...
            toStar: toStar,
            count: 1 // Each triangle represents 1 count
        });
    }
}
...
function updateMovingTriangles() {
    ...
    movingTriangles = movingTriangles.filter(group => {
        ...
        //const neighbors = movingTriangles.filter(t => t !== group && t.mesh.position.distanceTo(group.mesh.position) < neighborhoodRadius);
        const neighbors = movingTriangles.filter(t => t !== group && t.mesh.position.distanceTo(group.mesh.position) < 5);
        // Cohesion
        ...
        if (group.progress >= 1) {
            ...
            if (nextStar) {
                if (!nextStar.userData.isOwned) {
                    ...
                    // Reinforce owned star
                    const result = handleTriangleArrival(nextStar, group.count);
                    group.count = result.remainingTriangles;
                }
            }
            ...
            if (group.currentSegment >= group.path.length - 1 || group.count <= 0) {
                if (group.toStar) {
...
function handleTriangleArrival(star, count) {
    ...
    if (star.userData.isOwned) {
        const spaceAvailable = STAR_LIMITS[getStarType(star)] - star.userData.triangles;
        const trianglesToAdd = Math.min(count, spaceAvailable);
        star.userData.triangles += trianglesToAdd;
        console.log(`Reinforced star. New triangle count: ${star.userData.triangles}`);
        return { remainingTriangles: count - trianglesToAdd };
    } else {
